...
do
    ...
    do
        ...
        seqno=`lite-client -C ./ton-global.config -c 'runmethod '$user' seqno' |  grep 'remote result' | cut -d "[" -f2 | cut -d "]" -f1`
        fift -s fift_scripts/wallet.fif "build/new-wallet-0"$i $CONTRACT $seqno 1 "./build/wallet-query" -B "./build/buy-unit.boc" 2>&1
        res=` lite-client -v 0 -C ./ton-global.config -l /dev/null -c "runmethod $CONTRACT getunits -1 0x$user_hex $type $level" 2>&1 | grep result | cut -d "[" -f2 | cut -d "]" -f1`
        if [[ $res == " 0 " ]] 
        then
