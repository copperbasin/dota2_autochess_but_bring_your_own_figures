...
do
    ...
    seqno=`lite-client -v 0 -C ./ton-global.config -l /dev/null -c 'runmethod '$user' seqno' 2>&1 |  grep result | cut -d "[" -f2 | cut -d "]" -f1`
    player_idx=`lite-client -v 0 -C ./ton-global.config -l /dev/null -c 'runmethod '$CONTRACT' getplayeridx -1 0x'$user_hex 2>&1 |  grep result | cut -d "[" -f2 | cut -d "]" -f1 `
    ...
    lite-client -C ./ton-global.config -l null -c 'sendfile ./build/wallet-query.boc'
    sleep 4
done