...
SEQNO=`lite-client -v 0 -C ./ton-global.config -c  'runmethod '$USER' seqno' |  grep 'remote result' | cut -d "[" -f2 | cut -d "]" -f1`



fift -s fift_scripts/wallet.fif $WALLET_NAME $CONTRACT $SEQNO 0.5 "./build/wallet-query"
lite-client -v 0 -C ./ton-global.config -l /dev/null -c 'last'
lite-client -v 0 -C ./ton-global.config -l /dev/null -c 'sendfile ./build/wallet-query.boc'
sleep 5
lite-client -v 0 -C ./ton-global.config -l /dev/null -c 'last'
lite-client -v 0 -C ./ton-global.config -l /dev/null -c 'sendfile ./build/new-game-query.boc'